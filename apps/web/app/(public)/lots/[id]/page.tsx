...
export default async function LotDetailPage({ params }: LotPageProps) {
  ...
  // Broker phone is only set on the response when the API decides to reveal it.
  const brokerPhone = lot.broker?.phone ?? '';

  return (
    <div className="px-4 py-4">
      <div className="text-[11px] text-neutral-500">
        <span>Browse</span> / <span className="text-neutral-700">{formatVariety(lot.variety)}</span>
      </div>
      ...
      <div className="mt-3 space-y-4">
        ...
        {lot.photos.length > 1 && (
          <div className="grid grid-cols-4 gap-2">
            {lot.photos.slice(1).map((p, i) => (
              ...
              >
                <Image src={p} alt={`photo ${i + 2}`} fill sizes="25vw" className="object-cover" />
              </div>
            ))}
          </div>
        )}
        ...
        <h1 className="text-2xl font-bold tracking-tight">
          ...
          <span className="text-sm font-normal text-neutral-500">per quintal</span>
        </h1>
        ...
        <Card>
          <CardBody className="space-y-1 text-sm">
            ...
            <div className="text-neutral-500">Available from {formatDate(lot.available_from)}</div>
          </CardBody>
        </Card>
